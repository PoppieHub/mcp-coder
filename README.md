...
Для human-in-the-loop укажите `humanReview: true`. Первый вызов только исследует проект и возвращает `awaiting_approval` с полем `plan`, не меняя файлов. После одобрения передайте текст этого плана в `approvedPlan` при повторном вызове. Исполнение вернёт `ready_for_review` с diff и проверками: человек принимает или отклоняет изменения отдельно, до коммита.
